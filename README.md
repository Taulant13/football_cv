...
| **Fine-tuned YOLOv8s** (this work) | **95.5%** | mAP50 = 0.663, ~7 ms/frame on RTX 4060 |
...
The Kalman filter holds 4 numbers: `[x, y, vx, vy]`. Each frame it `predict()`s the next position, then `correct()`s with the new measurement. Velocity is never measured directly but inferred over time.

**Measurement gating.** Naïve Kalman blindly trusts whatever the detector hands it. In practice the fine-tuned detector occasionally fires a high-confidence false positive on a corner flag or shin guard, several hundred pixels from the real ball. Without gating, the tracker accepts the teleport and downstream speed estimates explode (observed peaks of 429 m/s on the first run). The fix is a single threshold: reject any measurement whose distance from the filter's prediction exceeds `MAX_JUMP_PIXELS = 150`. A legitimate 30 m/s shot at our camera zoom moves the ball ~50 px/frame, so the threshold is generous for real motion and tight against teleports.

After 15 consecutive missed or rejected frames the tracker resets. Otherwise stale predictions drift.

### Speed estimation

Consecutive tracker positions give pixel velocity. A one-time 2-click calibration (`calibrate_scale.py`) measures pixels-per-meter against a known feature in the frame (e.g. penalty spot - goalline = 11 m). Multiply by frame rate, smooth with a 5-frame moving average. The status overlay distinguishes three states: green **DETECTED**, red **rejected**, orange **predicted**.
...
All scripts are run as top-level modules from the project root (they resolve paths relative to `Path(__file__).parent.parent`, so **don't** use `python -m src.pipeline`):
...
After training, copy `runs/detect/train/weights/best.pt` to `models/yolov8_ball.pt` (A pre-trained `yolov8_ball.pt` is already shipped with this repo, so you can skip Step 3 to just run inference.)
...
**Split convention (load-bearing).** Train SNMOT-060..071, val SNMOT-072..073, test (never touched) SNMOT-074..075. Split is by sequence, not by frame, to prevent near-duplicate leakage. `prepare_dataset.py` also subsamples every 5th frame for the same reason.

**Per-sequence ball ID.** The ball's `trackletID` varies by sequence so read it from `gameinfo.ini` (key `trackletID_<n> = ball`), do not hard-code it.

## Design invariants

A few choices are non-obvious and load-bearing:

- **`imgsz=1280` for training and inference.** Non-negotiable for tiny-ball detection.
- **Single-target tracker by design.** The picker reduces N detections to one by argmax(confidence).
- **Measurement gating defends the tracker, not the detector.** If you tune `processNoiseCov` / `measurementNoiseCov`, re-verify gating still rejects teleports.
- **Reset after 15 missed frames.** Larger → stale predictions drift, smaller lets brief occlusions break the trail.
